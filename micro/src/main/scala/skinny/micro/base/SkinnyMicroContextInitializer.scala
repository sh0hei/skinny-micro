...
trait SkinnyMicroContextInitializer {
   ...
   */
  def skinnyMicroContext(implicit ctx: ServletContext): SkinnyMicroContext = {
  ...
  def context: SkinnyMicroContext = skinnyMicroContext(servletContext)

  def request(implicit ctx: SkinnyMicroContext = context): HttpServletRequest = ctx.request

  def response(implicit ctx: SkinnyMicroContext = context): HttpServletResponse = ctx.response
