...
trait FileUploadSupport
  ...
  def fileMultiParams(implicit ctx: SkinnyMicroContext): FileMultiParams = {
    extractMultipartParams(ctx.request).fileParams
  }

  def fileMultiParams(key: String)(implicit ctx: SkinnyMicroContext): Seq[FileItem] = {
   ...
   */
  def fileParams(implicit ctx: SkinnyMicroContext): MultiMapHeadView[String, FileItem] = {
    new MultiMapHeadView[String, FileItem] {
      protected def multiMap = fileMultiParams(ctx)
    }
  }

  def fileParams(key: String)(implicit ctx: SkinnyMicroContext): FileItem = {
    fileParams(ctx)(key)
  }
}
