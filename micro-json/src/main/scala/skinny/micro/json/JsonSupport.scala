...
trait JsonSupport[T] extends JsonOutput[T] {
  ...
  protected def parseRequestBody(format: String)(implicit ctx: SkinnyMicroContext) = try {
  ...
  protected def shouldParseBody(fmt: String)(
    implicit ctx: SkinnyMicroContext) = {
    (fmt == "json" || fmt == "xml") &&
      !ctx.request.requestMethod.isSafe &&
      parsedBody(ctx) == JNothing
  }

  def parsedBody(implicit ctx: SkinnyMicroContext): JValue = ctx.request.get(ParsedBodyKey).fold({
